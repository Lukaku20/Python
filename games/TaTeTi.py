...
def ingresaLetraJugador():
     #Permite al jugador tipear la letra que desea ser
...
def jugarDeNuevo():
    #Esta funcion devuelve TRUE(Verdadero) si el jugador quiere volver a jugar, de lo contrario devuelve FALSE(Falso)
...
def obtenerJugadaComputadora(tablero, letraComputadora):
    ...
    if letraComputadora == 'X':
        ...
        # Aqui esta nuestro algoritmo para nuestra IA (Inteligencia Artificial) Ta Te Ti.
    ...
    for i in range(1, 10):
        ...
        # Verifica si el jugadro puede ganar en la proxima jugada, y lo bloquea.
    for i in range(1, 10):
        copia = obtenerDuplicadoTablero(tablero)
        if hayEspacioLibre(copia, i):
        ...
        # Intenta ocupar una de las esquinas que esta libre.
